...
def xout(board, row, col):
    ...
    All spots where non-attacking queen can no
    ...
    for r in range(row + 1, len(board)):
        ...
        board[r][c] = "x"
        c += 1
    # X out all spot diagonally up to the left
    c = col - 1
    for r in range(row - 1, -1, -1):
        if c < 0:
            break
        board[r][c]
        c -= 1
    # X out all spot diagonally up to the right
    c = col + 1
    for r in range(row - 1, -1, -1):
        if c >= len(board):
            break
        board[r][c] = "x"
        c += 1
    # X out all spot diagonally down to the left
    c = col - 1
    for r in range(row + 1, len(board)):
        if c < 0:
            break
...
def recursive_solve(board, row, queens, solutions):
    """Recursively solve an N-queen puzle.
    Args:
        board (list): The current working chesboard.
